...
const ServiceManagement: React.FC = () => {
  ...
  const addServico = async (e: React.FormEvent) => {
    ...
    if (isNaN(precoNum) || precoNum <= 0) {
      console.warn("Validação falhou: preço inválido.");
    ...
    try {
      console.log("Dados do serviço:", { nome, preco: precoNum });
  ...
  const deleteServico = async (nome: string) => {
    console.log("Deletando serviço:", nome);
    try {
  ...
  const updateServico = async (nome: string, novoNome: string, novoPreco: number) => {
    console.log("Atualizando serviço:", nome);
    try {
